...
comp-kitchen-spike-deny-changeling = { CAPITALIZE(THE($victim)) } сопротивляется попытке насадить { OBJECT($victim) } на { THE($this) }.
comp-kitchen-spike-deny-absorbed = { CAPITALIZE(THE($victim)) } больше нечего разделывать.
...
comp-kitchen-spike-kill = { CAPITALIZE($user) } насаживает { $victim } на { $this }, тем самым убивая { SUBJECT($victim) }!
